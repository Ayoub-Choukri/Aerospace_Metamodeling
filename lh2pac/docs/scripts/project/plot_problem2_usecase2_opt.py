...
#%%
## Plotting the current values of the parametres of the aircraft
for i in range(len(List_Disciplines)):
    print(List_Disciplines[i].input_grammar.defaults)


#%%
## Changing to Hydrogen Aircraft
Dict_Parametres_To_Change = {}

Dict_Parametres_To_Change["fuel_type"] = "liquid_h2"
Dict_Parametres_To_Change["engine_type"] = "turbofan"
Dict_Parametres_To_Change["slst"] = 100000.001115062
Dict_Parametres_To_Change["n_pax"] = 120
Dict_Parametres_To_Change["area"] = 111.5844569300095
Dict_Parametres_To_Change["ar"] = 8.8913266205
...
class MyDesignSpace(DesignSpace):
    def __init__(self):
        ...
        self.add_variable("gi", lower_bound=0.35, upper_bound=0.405, value=0.4)
        self.add_variable("vi", lower_bound=0.35, upper_bound=0.405, value=0.4)
        self.add_variable("fc_pwd", lower_bound=0.8, upper_bound=1.02, value=1)
...
class MyUncertainSpace(ParameterSpace):
    def __init__(self):
        ...
        self.add_random_variable("gi", "OTTriangularDistribution", minimum=0.35, mode=0.4, maximum=0.405)
        self.add_random_variable("vi", "OTTriangularDistribution", minimum=0.35, mode=0.4, maximum=0.405)
        self.add_random_variable("fc_pwd", "OTTriangularDistribution", minimum=0.8, mode=1, maximum=1.02)
...
variable_names = ["cef", "sef", "aef", "mtom", "tofl", "vz", "fm", "vapp", "length", "gi", "vi", "fc_pwd"]
axes = axes.flatten()
for ax, name in zip(axes, variable_names):##### We find that the tofl constraint is not satisfied, so we will not plot the results here.
...
statistics = EmpiricalStatistics(data_simulation, ["mtom", "tofl", "vz", "span", "length", "fm", "vapp"]+["cef","aef","sef","gi","vi","fc_pwd"])
...
def Coefficient_Variation(mean, var):
    ...
    for variable in mean.keys():
        ...
        Coefficient_Variation_Variable = Std_Variable / np.abs(Mean_Variable) if Mean_Variable else 0
...
def IQ(data_simulation):
    ...
    q75 = {}
    IQ = {}
    for var in ["mtom", "tofl", "vz", "span", "length", "fm", "vapp", "cef", "aef", "sef", "gi", "vi", "fc_pwd"]:
...
print(f"Interquartile Range: {IQQ}")
