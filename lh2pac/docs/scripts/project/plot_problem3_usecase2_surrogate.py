"""
Problem 3 Use Case 2 Iterative Surrogate Method
This script creates ONE surrogate model for each iteration for the aircraft design problem and optimizes it with the surrogate-based approach
"""
...
scenario3 = add_constraint_to_scenario(scenario3)

scenario3.execute(algo_name="NLOPT_COBYLA", max_iter=200)

#%% Check resultats and optimisation evolution 

result3 = scenario3.optimization_result
scenario3.post_process(post_name="OptHistoryView", save=False, show=False)
...
slst3, n_pax3, area3, ar3 = result3.x_opt
ls = [slst3, n_pax3, area3, ar3]
for i in ls: 
    print(i)
...
config_opt3 = AircraftConfiguration(
    name="Opt_iterative_Surrogate", 
    area=area3, 
    length=32, 
    n_pax=np.round(n_pax3), 
    slst=slst3,
    color="gray"
)
draw_aircraft(config_opt, config_opt3)
#%% 